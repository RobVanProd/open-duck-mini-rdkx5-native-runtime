...
| BNO055 + contacts | transactional guarded `calibrate_imu`; independent capture re-verifier; fail-closed nine-label launcher; guarded labeled `probe_sensors`; chip-ID and exact calibration readback; per-label and matrix validators | capture authorization/timeout/atomicity/schema/readback tests; restricted conversion, full status re-derivation, runner sequencing, raw-unit, axis-remap, polarity, freshness, provenance, tamper, and guard tests | physical calibration complete with 1,318 rows, five terminal `0xff` samples, exact readback, profile SHA `e7518b0d…`; BCM22→GPIO388 / BCM27→GPIO379 verified; labeled matrix `NOT_RUN` |
...
| Staged authority boundary | dual CLI assertions, movement/Gate-5-specific assertions, finite exact-command replay, frozen Gate 2 launcher, runbook, `NOT_RUN` files | pre-I/O guard, source/config freeze, stage-order, and summary-validator tests | Gate 2 `PASS_REVIEWED`; Gates 3-5 remain unauthorized/`NOT_RUN` |
