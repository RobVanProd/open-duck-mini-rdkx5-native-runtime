...
4. one final `runtime_halt` event with the halt reason and drop count.

The startup event freezes the runtime contract, config and policy SHA-256,
101/14 ONNX names and shapes, joint/servo order, command, total and active tick
caps, bus settings, controller, RT requirement, and hardware assertions. The
...
raw records. The summary includes the raw JSONL SHA-256.
