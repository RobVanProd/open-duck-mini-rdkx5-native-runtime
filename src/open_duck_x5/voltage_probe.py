...
from .bus.sts3215 import ADDR_PRESENT_VOLTAGE
...
def _finding(records: list[dict[str, object]], complete: bool) -> str:
    if not complete:
        return "incomplete_transport"
    voltage_errors = [bool(record["input_voltage_error"]) for record in records]
    if all(voltage_errors):
        return "all_servos_input_voltage_error"
    if any(voltage_errors):
        return "mixed_input_voltage_error"
    return "no_input_voltage_error"
...
def run_probe(args: argparse.Namespace) -> dict[str, object]:
    ...
    if args.bus == "serial":
        require_hardware_authorization(
            ...
            operation="all-servo present-voltage diagnostic",
        )
        if not COMMIT_RE.fullmatch(args.repository_commit):
            raise ValueError("serial voltage diagnostic requires a 40-hex repository commit")
    ...
    try:
        for wire_index, servo_id in enumerate(SERVO_SYNC_READ_IDS):
            ...
            status, device_error, parameters = bus.read_register_with_device_status(
                servo_id, ADDR_PRESENT_VOLTAGE, 1
            )
            round_trip_ns = clock_ns() - start_ns
            complete_response = (
                status in (ErrorCode.OK, ErrorCode.DEVICE)
                and device_error is not None
                and len(parameters) == 1
            )
            voltage_raw = int(parameters[0]) if complete_response else None
            records.append(
                {
                    "wire_index": wire_index,
                    "logical_index": SERVO_IDS.index(servo_id),
                    "servo_id": servo_id,
                    "register_address": ADDR_PRESENT_VOLTAGE,
                    "response_status": status.name.lower(),
                    "device_status_raw": device_error,
                    "input_voltage_error": bool(
                        device_error is not None and device_error & 0x01
                    ),
                    "voltage_raw": voltage_raw,
                    "present_voltage_v": voltage_raw * 0.1 if voltage_raw is not None else None,
            ...
            if complete_response:
                ...
                if consecutive_transport_failures >= args.watchdog_failures:
                    halt_reason = (
                        "consecutive voltage-read transport failures reached "
                        f"{args.watchdog_failures}"
                    )
                    break
    except Exception as exc:
        halt_reason = f"voltage diagnostic exception: {exc}"
    finally:
        try:
    ...
    summary = {
        "schema_version": "open_duck_x5.voltage_diagnostic.v1",
        "backend": args.bus,
        ...
        "finding": _finding(records, complete_all14),
        "register_address": ADDR_PRESENT_VOLTAGE,
        "volts_per_count": 0.1,
