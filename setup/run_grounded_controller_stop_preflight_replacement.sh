...
readonly expected_preregistration_sha256="4d031b0d63c87e08cc77e8df799b41325f2271223885bf9e2c9bb36eeb4edf8e"
readonly expected_attempt1_sha256="3e3a44bac1aba8b00dae05e7a1a647d7f412db352065bac02cabf558db167969"
...
usage() {
  cat <<'EOF'
Usage: setup/run_grounded_controller_stop_preflight_replacement.sh \
  --source-root /home/sunrise/open-duck-x5-controller-stop-replacement \
  --output-dir /home/sunrise/duck-evidence/controller-b-stop-replacement-YYYYMMDD \
  --hardware-authorized --suspended-or-benched

Replacement controller-only B-button mapping probe after the reviewed tick-0
timestamp-ordering halt. It opens /dev/input/js0 only. It does not open serial,
access servos, enable torque, load a policy, or command motion.
EOF
}
...
if [[ "$(git -C "$source_root" rev-parse HEAD:src/open_duck_x5)" != "$expected_source_tree" || \
  ...
  echo "result=BLOCKED reason=frozen_replacement_source_mismatch" >&2
  exit 2
fi
if ! git -C "$source_root" merge-base --is-ancestor "$expected_source_commit" HEAD; then
  echo "result=BLOCKED reason=replacement_source_commit_not_ancestor" >&2
  exit 2
fi

require_hash \
  "$source_root/artifacts/gates/grounded_validation/CONTROLLER_B_STOP_NO_SERVO_REPLACEMENT_PREREGISTRATION_20260802.json" \
  "$expected_preregistration_sha256" "replacement_preregistration"
require_hash \
  "$source_root/artifacts/gates/grounded_validation/CONTROLLER_B_STOP_NO_SERVO_ATTEMPT1_HALTED_20260802.json" \
  "$expected_attempt1_sha256" "attempt1_halt_receipt"
require_hash \
...
checks = {
    ...
    "sample_check_order": all(
        int(record["sample_checked_monotonic_ns"])
...
payload = {
    "schema_version": "open_duck_x5.controller_b_stop_no_servo_replacement_validation.v1",
...
payload = {
    "schema_version": "open_duck_x5.controller_b_stop_no_servo_replacement_run.v1",
