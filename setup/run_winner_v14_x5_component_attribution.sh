...
readonly staging_root_expected="/home/sunrise/open_duck_x5_preflight/t251a3"
readonly rt_cpu=7
...
usage() {
  cat <<'EOF'
Usage: sudo bash setup/run_winner_v14_x5_component_attribution.sh \
  --staging-root /home/sunrise/open_duck_x5_preflight/t251a3 \
  --python /home/sunrise/open_duck_x5_preflight/t251a3/venv/bin/python \
  --x5-cpu-attribution-authorized --no-robot-device-access

Runs only the preregistered T251A3 synthetic-state single-host component
attribution. It exposes no serial, sensor, controller, GPIO, I2C, servo,
...
(
  ...
  exec taskset -c "$rt_cpu" chrt -f "$rt_priority" \
    ...
    tools/run_winner_v14_x5_component_attribution.py \
...
if ((runner_status != 0)); then
  echo "result=HOLD reason=t251a3_runner_failed evidence=$evidence_dir" >&2
  exit 2
fi
