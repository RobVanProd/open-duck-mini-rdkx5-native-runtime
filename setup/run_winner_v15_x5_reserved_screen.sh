...
readonly staging_root_expected="/home/sunrise/open_duck_x5_preflight/t251a4"
readonly rt_cpu=7
...
usage() {
  cat <<'EOF'
Usage: sudo bash setup/run_winner_v15_x5_reserved_screen.sh \
  --staging-root /home/sunrise/open_duck_x5_preflight/t251a4 \
  --python /home/sunrise/open_duck_x5_preflight/t251a4/venv/bin/python \
  --x5-cpu-screen-authorized --no-robot-device-access

Runs only the preregistered T251A4 synthetic-state semantic and single-host
timing screen. It exposes no serial, sensor, controller, GPIO, I2C, servo,
torque, or motion path and restores the exact prior CPU governor.
EOF
}
...
taskset -c "$rt_cpu" chrt -f "$rt_priority" \
  "$python_path" "$source_root/tools/run_winner_v15_x5_reserved_screen.py" \
...
if ((runner_status != 0)); then
  echo "result=HOLD reason=t251a4_runner_failed status=$runner_status" >&2
  exit "$runner_status"
fi
