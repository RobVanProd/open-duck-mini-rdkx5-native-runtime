...
readonly staging_root_expected="/home/sunrise/open_duck_x5_preflight/t251a2"
readonly rt_cpu=7
...
usage() {
  cat <<'EOF'
Usage: sudo bash setup/run_winner_v14_x5_paced_screen.sh \
  --staging-root /home/sunrise/open_duck_x5_preflight/t251a2 \
  --python /home/sunrise/open_duck_x5_preflight/t251a2/venv/bin/python \
  --x5-cpu-screen-authorized --no-robot-device-access

Runs only the preregistered T251A2 synthetic-state optimized paced CPU screen.
It exposes no serial, sensor, controller, GPIO, I2C, servo, torque, or motion
path. It changes policy0 to performance for the run and restores the exact
prior governor on every exit path.
EOF
}
...
(
  ...
  exec taskset -c "$rt_cpu" chrt -f "$rt_priority" \
    ...
    tools/run_winner_v14_x5_paced_screen.py \
      ...
      --output "$result_path" \
      --raw-output "$raw_path"
) > "$evidence_dir/screen-stdout.txt" \
  2> "$evidence_dir/screen-stderr.txt"
runner_status=$?
set -e

if ((runner_status != 0)); then
  echo "result=HOLD reason=t251a2_runner_failed evidence=$evidence_dir" >&2
  exit 2
fi
