...
| Winner-v2 runtime-v2 | Separate/default-disabled 115-D implementation passes all 2,400 offline ticks and Windows/Linux CPU reviews agree on bit-exact STS wire closure; a reporting-only exact-observation correction is committed, while the prior asset lock is held stale pending policy revalidation; physical COM, policy clearance, and no-servo X5 CPU replay remain |
